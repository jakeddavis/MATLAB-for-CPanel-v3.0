...
% 1 = completely inside
% 2 = partially inside -> check edge data
% 3 = pure edges or outside -> check edge data

% Edge Data Info
% Ex: [1 0 1] means edges 1 and 3 are intersected, 2 is outside


function [depFlag, edgeData] = supDoDCheck_10_13(M,verts,P,cntr)
...
%         %%%%%%%%%%%%% don't need
        ...
        for j = 1:3
            pnts = supIntersectionPnt(P,loopVerts(j,:),loopVerts(j+1,:),B);
            [nPnts,~] = size(pnts);
            for i = 1:nPnts
                plot(pnts(i,1),pnts(i,2),'*r')
            end
        end
        xiMin = x - sqrt(B^2*z^2);
        plot(xiMin,y,'m*')
...
%             edgeData(:,:) = 1;
        elseif count == 1
            ...
            if pntInterData(1) == 1
                edgeData(1) = 1;
                edgeData(3) = 1;
                edgeData(2) = supEdgeCheck(verts(2,:),verts(3,:),P);
            elseif pntInterData(2) == 1
                edgeData(1) = 1;
                edgeData(2) = 1;
                edgeData(3) = supEdgeCheck(verts(3,:),verts(1,:),P);
            elseif pntInterData(3) == 1
                edgeData(2) = 1;
                edgeData(3) = 1;
                edgeData(1) = supEdgeCheck(verts(1,:),verts(2,:),P);
            end
        elseif count == 2
            depFlag = 2;
