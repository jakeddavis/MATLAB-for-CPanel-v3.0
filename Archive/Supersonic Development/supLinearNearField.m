...
% Jake Davis
% 09/10/2018 - generalized to include source calc

function [phiD, phiS] = supLinearNearField(Mach,mu,sigma,verts,P,cntr)

[depFlag, newVerts, PSup] = supNewPanel(verts,P,cntr,Mach);
...
if depFlag == 0
    ...
    if depFlag == 1 || depFlag == 2
        % Completely inside, or pnt(s) inside
        ...
        [M,~] = size(newVerts);
        newVerts(M+1,:) = newVerts(1,:);
        for i = 1:M
            pnt1 = [newVerts(i,1) newVerts(i,2)];
            pnt2 = [newVerts(i+1,1) newVerts(i+1,2)];
            
            [Q1temp, w0temp, xm, m] = supInfCoeffs(pnt1,pnt2,PSup,delMag,depFlag);
