...
function [Q1, w0, xm, m] = supInfCoeffs(pnt1,pnt2,P,c0,delMag)
...
% Check if vertices are upstream of field point and inside Mach cone
R1 = 0;
R2 = 0;
if dot(P' - pnt1, c0) >= 0
    R1 = real(sqrt(sm1^2 - r1^2));
end
if dot(P' - pnt2, c0) >= 0
    R2 = real(sqrt(sm2^2 - r2^2));
end
% if xm >= 0
%     R1 = real(sqrt(sm1^2 - r1^2));
%     R2 = real(sqrt(sm2^2 - r2^2));
...
%     if abs(z) > .00000000001
        if abs(1 - abs(m)) < delSmall % sonic edge
            [Q1,w0] = supEdgeInfSon(xm,ym1,ym2,R1,R2,lam,z,delSmall);
...
%             [Q1,w0] = supEdgeInfInPlaneSon(R1,R2,ym1,ym2,lam,z,delSmall);
        elseif abs(m) < 1 % subsonic edge
            [Q1,w0] = supEdgeInfInPlaneSub(R1,R2,xmc,ym1c,ym2c,m,z,delSmall,mFlag);
        elseif abs(m) > 1 % supersonic edge
            [Q1,w0] = supEdgeInfInPlaneSup(R1,R2,xm,ym1,ym2,lam,z,delSmall);
        end
    end
elseif (abs(m) > 1) 
%     xmInter = (interPnt(1)-x1) - (interPnt(2)-y1)*lam;
    wedgeCheck = xm^2 + z^2/m^2 - z^2;
    if (xm > 0) && (sign(ym1) ~= sign(ym2)) && (abs(ym1) > 0.0001 && abs(ym2) > 0.0001 && (wedgeCheck >= 0))
%     if (sign(ym1) ~= sign(ym2)) && (abs(ym1) > 0.0001 && abs(ym2) > 0.0001) && (wedgeCheck >= 0)
        Q11 = sign(z*ym1) * (pi/2);
